...
namespace cells_db.model
{
    public class Detail
    {
        ...
        public List<ResearchResult> ResearchResults { get; set; }
        ...
        public Detail(int id, Material material, CellStructure cS, string name, string description, List<Property> property, List<ResearchResult> researchResults)
        {
            Id = id;
            Material = material;
            CellStructure = cS;
        ...
        {
            StringBuilder sb = new StringBuilder();           
            foreach (ResearchResult result in this.ResearchResults)
            {
                sb.Append(result).Append(Environment.NewLine);
            }
            return sb.ToString();
        }
