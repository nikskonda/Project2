...
namespace cells_db.sql
{
    class DBCreater
    {
        ...
        public DBCreater(String name, String path)
        {
            ...
            result = "USE[" + name + "] SET ANSI_NULLS ON SET QUOTED_IDENTIFIER ON CREATE TABLE[dbo].[ResearchResult] (  [Id_result][int] IDENTITY(1,1) NOT NULL,   [Id_Detail] [int] NOT NULL, [Name] [varchar](50) NOT NULL,  [Parameter] [varchar] (1000) NOT NULL,   [Iteration] [int] NOT NULL, " +
            " [Stress] [float] NOT NULL,  [Displacement] [float] NOT NULL, CONSTRAINT[PK_ResearchResult] PRIMARY KEY CLUSTERED([Id_result] ASC )WITH(PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON[PRIMARY] ) ON[PRIMARY]";
